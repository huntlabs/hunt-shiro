 ...
 * Shiro's Hash implementations (for example, {@link hunt.shiro.crypto.hash.Md5Hash Md5Hash},
 * {@link hunt.shiro.crypto.hash.Sha1Hash Sha1Hash}, etc) support salting and multiple hash iterations via
 ...
 * {@link #getSalt(hunt.shiro.authc.AuthenticationToken) getSalt(AuthenticationToken)} method.  This however
 ...
 * {@link hunt.shiro.authc.SaltedAuthenticationInfo#getCredentialsSalt credentialsSalt} for hashing.  To avoid
 * security risks,
 ...
 *
 * @see hunt.shiro.crypto.hash.Md5Hash
 * @see hunt.shiro.crypto.hash.Sha1Hash
 * @see hunt.shiro.crypto.hash.Sha256Hash
 * @since 0.9
 */
class HashedCredentialsMatcher : SimpleCredentialsMatcher {
    ...
    private bool hashSalted;
    private bool storedCredentialsHexEncoded;
    ...
    /**
     * Returns the {@code Hash} {@link hunt.shiro.crypto.hash.Hash#getAlgorithmName() algorithmName} to use
    ...
    /**
     * Sets the {@code Hash} {@link hunt.shiro.crypto.hash.Hash#getAlgorithmName() algorithmName} to use
     * when performing hashes for credentials matching.
     *
     * @param hashAlgorithmName the {@code Hash} {@link hunt.shiro.crypto.hash.Hash#getAlgorithmName() algorithmName}
     *                          to use when performing hashes for credentials matching.
     ...
     * <p/>
     * If enabled, the salt used will be obtained via the {@link #getSalt(hunt.shiro.authc.AuthenticationToken) getCredentialsSalt} method.
     * </p>
     * The default value is {@code false}.
     *
    ...
    //     if (credentialsCast !is null || credentials instanceof[] char) {
     ...
     * {@link hunt.shiro.authc.SaltedAuthenticationInfo SaltedAuthenticationInfo}.  It then compares the hash
     * against the {@code AuthenticationInfo}'s
     * {@link #getCredentials(hunt.shiro.authc.AuthenticationInfo) already-hashed credentials}.  This method
     ...
     * {@link SimpleCredentialsMatcher#getCredentials(hunt.shiro.authc.AuthenticationInfo) getCredentials(AuthenticationInfo)} implementation.
     *
