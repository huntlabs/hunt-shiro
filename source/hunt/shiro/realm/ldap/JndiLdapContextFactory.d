 ...
 * under the License.
 */
module hunt.shiro.realm.ldap.JndiLdapContextFactory;
 ...
 * @since 1.1
 */
class JndiLdapContextFactory : LdapContextFactory {
    ...
    protected enum string DEFAULT_CONTEXT_FACTORY_CLASS_NAME = "com.sun.jndi.ldap.LdapCtxFactory";
    protected enum string SIMPLE_AUTHENTICATION_MECHANISM_NAME = "simple";
    protected enum string DEFAULT_REFERRAL = "follow";
     ...
     *         account principal, {@code false} otherwise.
     */
    protected bool isPoolingConnections(Object principal) {
        return isPoolingEnabled() && principal != null && principal== getSystemUsername();
    }
     ...
     */
     LdapContext getLdapContext(Object principal, Object credentials)
        ...
        if (authcMech  is null && (principal != null || credentials != null)) {
        ...
        bool pooling = isPoolingConnections(principal);
        if (pooling) {
            env.put(SUN_CONNECTION_POOLING_PROPERTY, "true");
        }

        if (log.isDebugEnabled()) {
            tracef("Initializing LDAP context using URL [{}] and principal [{}] with pooling {}",
